...
class KeyMap(object):
	...
	def _addSingle(self, mapping={}, keys=[], mods=[], event=None, function=None, state=1):
		...
		if local_mapping["keys"] and (local_mapping["function"] or local_mapping["event"]):
			...
			local_mapping["keys"] = set(list(local_mapping["keys"]) if type(local_mapping["keys"]) == list else [local_mapping["keys"]])
	...
	def add(self, mapping={}, keys=[], mods=[], event=None, function=None, state=1):
		if type(mapping) == list:
			[self._addSingle(m, keys, mods, event, function, state) for m in mapping]
		else:
			self._addSingle(mapping, keys, mods, event, function, state)
	...
	def getModsSets(self, mods):
		...
		for mod in mods:
			if "meta" in mod:
				...
				with_alt.add(mod)
				mod = mod.replace('left', '').replace('right', '')
				nutral.add(mod)
	...
	def getEvent(self, keys, mods=[], state=1):
		"""Gets the event that is triggered by the list of key and mods."""
		keys = set(list(keys) if type(keys) == list else [keys])
		mods = set(list(mods if type(mods) == list else [mods]))
		for mapping in self._keymap:
			if mapping['keys'] == keys and mapping['mods'] == mods and mapping['state'] == state:
				if mapping['function']:
				...
				for mod_set in mod_sets:
					if mapping['keys'] == keys and mapping['mods'] == mod_set and mapping['state'] == state:
						if mapping['function']:
							return mapping['function']()
						return mapping['event']
...
if __name__ == '__main__':
	keymap = KeyMap([
		{'keys':['space'], 'event': "oranges", 'state':0},
		{'keys':['f', 'a'], 'mods': ['ctrl','shift'], 'function': lambda: True},
	])
	print(keymap.getEvent(keys='space', state=0))